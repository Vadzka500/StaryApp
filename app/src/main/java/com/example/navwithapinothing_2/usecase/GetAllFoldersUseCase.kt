package com.example.navwithapinothing_2.usecase

import com.example.navwithapinothing_2.data.MovieDatabaseRepository
import com.example.navwithapinothing_2.data.ResultDb
import com.example.navwithapinothing_2.database.models.FolderWithMovies
import kotlinx.coroutines.flow.Flow
import javax.inject.Inject

/**
 * @Author: Vadim
 * @Date: 23.06.2025
 */
class GetAllFoldersUseCase @Inject constructor(private val repository: MovieDatabaseRepository) {
    operator fun invoke(): Flow<ResultDb<List<FolderWithMovies>>> = repository.getFolders()
}