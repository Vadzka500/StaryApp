...
class GetListMoviesByCollectionUseCaseNew @Inject constructor(private val repository: HomeRepository) {
    operator fun invoke(slug: String, limit: Int): Flow<Result<List<Movie>>> {
        return repository.getCollectionWithMovies(slug, limit)
    }
}
